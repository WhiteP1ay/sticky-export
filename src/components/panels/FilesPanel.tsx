    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="flex items-center justify-between gap-2">
          ...
          <span className="text-[11px] text-slate-500">{files.length ? `共 ${files.length} 个` : '暂无文件'}</span>
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="max-h-56 space-y-1 overflow-y-auto text-xs">
          {files.length === 0 ? (
            <div className="flex items-center justify-center py-6 text-[11px] text-slate-500">还没有导入 .tgs 文件</div>
          ) : (
            files.map((file) => (
              <button
                key={file.id}
              ...
              >
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <div className="truncate text-xs text-slate-100">{file.name}</div>
                    <div className="mt-0.5 flex items-center gap-2 text-[10px] text-slate-500">
                      <span>{renderStatus(file.status)}</span>
                      {file.error ? <span className="truncate text-red-400">{file.error}</span> : null}
                    </div>
                  </div>
                </div>
