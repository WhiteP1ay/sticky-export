  ...
  return (
    <Card className="flex flex-1 flex-col">
      <CardHeader>
        ...
        <CardDescription>根据当前导出配置（背景色、分辨率）进行预览</CardDescription>
      </CardHeader>
      <CardContent className="flex flex-1 flex-col gap-3">
        ...
        <div className="flex items-center justify-between gap-2 text-[11px] text-slate-400">
          <div className="flex items-center gap-2">
            <Button type="button" size="sm" variant="subtle" disabled={!selectedFile?.parsed} onClick={togglePlay}>
              <Play className="mr-1 h-3 w-3" />
              {isPlaying ? '暂停' : '播放'}
            </Button>
            <Button type="button" size="sm" variant="ghost" disabled={!selectedFile?.parsed} onClick={restart}>
          ...
          {selectedFile?.parsed ? (
            <span>
              {selectedFile.name} · {Math.round(selectedFile.parsed.durationMs / 100) / 10}s · 总帧数{' '}
              {selectedFile.parsed.totalFrames}
            </span>
          ) : null}
        </div>
      </CardContent>
